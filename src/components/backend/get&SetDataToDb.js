...
export function getRoomsFromDb(id, setRooms) {
  ...
  return db
    ...
    .onSnapshot((snapshot) => {
      ...
      let roomsInfo = snapshot.docs.map((doc) => ({
        id: doc.id,
        data: doc.data(),
      }));
      db.collection("rooms")
        ...
        .then((querySnapshot) => {
          ...
          roomsInfo.forEach((room) => {
            // loop through all the rooms in db and pull out the ones where user belong to
            array = [...array, ...datas.filter((data) => data.id === room.id)];
          });
          array.forEach((arr, arrIndex) => {
            // loop through room info and merge with user room info
    ...
    try {
      users.forEach((user, index) => {
        ...
        if (index === users.length - 1)
          hookCallback && hookCallback({ loading: false, success: true });
      });
    } catch (e) {
      console.log(e);
...
export function resetUserRoomOnScreenInDb(userId, roomId, value) {
  db.collection("rooms").doc(roomId).collection("members").doc(userId).update({
    isOnScreen: value,
  });
}
export function uploadFileToDb(
  file,
  fileInfo,
  setFileOnPreview,
...
export function registerNewUserInDb(email, phoneNumber, uid, name, avi) {
  ...
  const devData1 = {
    id: "i5U5QHp42TTlkiAELRENCojZwya2",
    email: "novahathi@gmail.com",
    phoneNumber: "+919999999999",
  };
  const deveData2 = {
    info: { uid: devData1.id, email: devData1.email },
    phoneNumber: devData1.phoneNumber,
  };
  const roomsId = [
    "-Mq9_HgciAxnFWRrvvSD",
    "-Mq9gWeyzZNXEo4xNnu_",
...
export function getTotalUsersFromDb(reducerCallback) {
  ...
  return db.collection("registeredUsers").onSnapshot((snapshot) => {
...
export function setMemberAdminStatusInDb(roomId, memId, admin) {
  ...
  db.collection("rooms").doc(roomId).collection("members").doc(memId).update({
    isAdmin: admin,
  });
}
export function removeMemberFromRoomInDb(roomId, memId) {
  ...
  db.collection("registeredUsers") // remove in members db rooms info
    ...
    .delete()
    .then(() => {
      // then remove in rooms members info
      db.collection("rooms")
        .doc(roomId)
        .collection("members")
        .doc(memId)
...
export function getIfMessageHasBeenReadFromDb(
  receiverId,
  senderId,
  mssgId,
  reactHookCallback
...
export function getIsUserTypingFromDb(
  userId,
  convoId,
  isRoom,
  reactHookCallback
...
export function resetIsUserOnDarkModeOnDb(userId, value) {
  ...
  db.collection("registeredUsers").doc(userId).update({
    darkMode: value,
  });
}
