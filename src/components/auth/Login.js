...
function Login() {
  ...
  useEffect(() => {
    const bg1Timeline = gsap.timeline({
      defaults: { duration: 6, ease: "power1" },
    });
    const bg2Timeline = gsap.timeline({
      defaults: { duration: 6, ease: "power1" },
    });

    bg1Timeline
      .to(".auth-bg1", { right: "50px", top: "20%" })
  ...
  useEffect(() => {
    // Gets the total registertered user on db and reset Path and reset url path
    history.push("/");
      ...
      {userAlreadyHasAcct ? (
        ...
        <div className="login">
          <div className="login__container">
            ...
            />
            <p className="login__eg">e.g +91 99999 99999</p>
            <Button
              onClick={() =>
                sign.up(phoneInput, totalUserOnDb, dispatch, history)
              }
            >
              Sign Up With Google
            </Button>
