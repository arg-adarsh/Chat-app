...
import {
  displayConvoForMobile,
  mobileDisplayConvoProfile,
} from "../utils/mobileScreenUtils";
import {
...
import {
  addParticipantAnimation,
  getChatThatAreNotMembers,
  isLoggedInUserAdmin,
  handleRemoveParticipant,
  clickedRoomMember,
  add,
} from "../utils/displayModalUtils";
...
function DisplayModal(props) {
  ...
  const [successSP, setSuccessSP] = useState({
    loading: false,
    success: false,
  }); // keeps loading and success state when selectedParticipant is being added in db
    ...
    return (
      <div>
        ...
        >
          <Fade in={openModal}>
            <div
              className={`${classes.paper} ${isUserOnDarkMode && "dark-mode1"}`}
            >
            ...
            {isRoom ? (
              <div
                className={`${classes.paper} ${
                  isUserOnDarkMode && "dark-mode1"
                }`}
              >
                <div className="modal__clearMssgs">
                  <p className={isUserOnDarkMode && "dark-mode-color1"}>
                    Sorry You Can't Clear Group chat Messages
                  </p>
        ...
        >
          <Fade in={openModal}>
            <div
              className={`${classes.paper} ${
                isUserOnDarkMode && "dark-mode1"
              } modal__addParticipant_wr`}
            >
              <div className="modal__addParticipant">
                <section
                  className={`modal_addparticipant_header ${
                ...
                <section
                  className={`modal_addparticipant_search ${
                    isUserOnDarkMode && "dark-mode1"
                  }`}
                >
                  <div
                    className={`modal_addparticipant_searchContainer ${
                      isUserOnDarkMode && "dark-mode2"
                    }`}
                  >
                    <div className="icons">
                ...
                <section
                  className={`modal_addparticipant_selectedWr ${
                    isUserOnDarkMode && "dark-mode2"
                  } `}
                >
                ...
                <section
                  className={`modal_addparticipant_membersWr ${
                    isUserOnDarkMode && "dark-mode2"
                  }`}
                >
                  <div className="modal_addparticipant_members">
                    <h3>Contacts</h3>
                    {getChatThatAreNotMembers(currentUserChats, roomMembers)
                      .length > 0 ? (
                      getChatThatAreNotMembers(
                        currentUserChats,
                        roomMembers
                      ...
                      <p className={isUserOnDarkMode ? "dark-mode-color1" : ""}>
                        You currently dont have any active chats
                      </p>
                    )}
                  </div>
                </section>
                {selectedParticipant.length > 0 && (
                  <CheckRounded
                    onClick={() => setModalType("A_P_CONFIRMATION")}
                  />
                )}
              </div>
            </div>
      ...
      <div>
        ...
        >
          <Fade in={openModal}>
            ...
            >
              <div
                className={`modal__aPConfirmation ${
                  successSP?.loading && "loading"
                }`}
              >
                {successSP.loading ? (
                  <Loading
                  ...
                  <>
                    <p className={`${isUserOnDarkMode && "dark-mode-color1"}`}>
                      {`${selectedParticipant
                        ?.map((participant) => participant?.info?.name)
                        .toString()} has been added to "${
                        currentDisplayConvoInfo?.roomName
                      }" Group Successfully`}
                    </p>
                    <button
                  ...
                  <>
                    ...
                    <button onClick={() => setModalType("ADD_PARTICIPANT")}>
                      CANCEL
                    </button>
                  </>
                )}
